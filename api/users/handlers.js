...
module.exports = (server) => {
  ...
  return {
    ...
    loginUser (request, reply) {
      ...
      //     return reply({
      //       errors: {
      //         404: ['email/user belum terdaftar!']
      //       }
      //     }).code(404)
      //   }
      ...
      //     return reply({
      //       errors: {
      //         'email or password': ['email atau password tidak cocok!']
      //       }
      //     }).code(401)
      //   }
      ...
      server.methods.services.users.getByUsername(payload.user.username, (err, user) => {
        if (err) return reply(replyHelper.constructErrorResponse(err)).code(422)
